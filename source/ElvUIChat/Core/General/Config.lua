...
function E:Config_DialogOpened(name)
	if name ~= 'ElvUIChat' then return end
...
function E:Config_CreateLeftButtons(frame, unskinned, options)
	...
	for index, opt in ipairs(opts) do
		...
		info.func = function()
			...
			if ACD then ACD:SelectGroup('ElvUIChat', key) end
		end
...
function E:Config_OpenWindow()
	local ACD = E.Libs.AceConfigDialog
	if ACD then ACD:Open('ElvUIChat') end
...
function E:Config_CreateBottomButtons(frame, unskinned)
	local L = E.Libs.ACL:GetLocale('ElvUIChat', E.global.general.locale or 'enUS')
		...
		if empty then
			...
			if pageCount > 1 then
				...
				for i = 1, pageCount do
					if i == 1 then
						main = pages[i] and ACD and ACD.Status and ACD.Status.ElvUIChat
				...
				local main = pages[1] and ACD and ACD.Status and ACD.Status.ElvUIChat
				mainSel = main and main.status and main.status.groups and main.status.groups.selected
			end
