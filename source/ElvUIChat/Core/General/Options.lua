...
function E:LoadConfigOptions_Chat()
    ...
    General.args.classColorMentionGroup.args.classColorMentionsChat = ACH:Toggle('Chat', 'Use class color for the names of players when they are mentioned.', 1, nil, nil, nil, function(info) return E.db.chat[info[#info]] end, function(info, value) E.db.chat[info[#info]] = value end)
    General.args.classColorMentionGroup.args.classColorMentionExcludeName = ACH:Input('Exclude Name', 'Excluded names will not be class colored.', 2, nil, nil, C.Blank, function(_, value) if value == '' or gsub(value, '%s+', '') == '' then return end E.global.chat.classColorMentionExcludedNames[strlower(value)] = value end)
    General.args.classColorMentionGroup.args.classColorMentionExcludedNames = ACH:MultiSelect('Excluded Names', nil, 3, function(info) return E.global.chat[info[#info]] end, nil, nil, C.Blank, function(info, value) E.global.chat[info[#info]][value] = nil GameTooltip_Hide() end)
