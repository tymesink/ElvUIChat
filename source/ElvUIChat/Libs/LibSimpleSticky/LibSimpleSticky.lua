...
------------------------------------------------------------------------------------
This is a modified version by Elv and Simpy for ElvUIChat
------------------------------------------------------------------------------------]]
...
-- GLOBALS: WorldFrame, UIParent, ElvUIChatParent
...
function StickyFrames:SnapFrame(frameA, frameB, left, top, right, bottom)
	...
	if (bA <= tB and bB <= tA) then
		...
		if lA <= (lB + StickyFrames.rangeX) and lA >= (lB - StickyFrames.rangeX) then
			newX = lB + wA
			if frameB == UIParent or frameB == WorldFrame or frameB == ElvUIChatParent then
				newX = newX + 4
			end
			snap = true
		end
