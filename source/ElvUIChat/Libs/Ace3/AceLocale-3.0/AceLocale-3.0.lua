...
-- @name AceLocale-3.0
-- @release $Id$
local MAJOR,MINOR = "AceLocale-3.0-ElvUIChat", 8
...
-- ElvUIChat modded by Darth
local readmeta = {
	__index = function(self, key) -- requesting totally unknown entries: fire off a nonbreaking error and return key
		AceLocale.MissingLinesElvUIChat[key] = true
		rawset(self, key, key)      -- only need to see the warning once, really
		geterrorhandler()(MAJOR..": "..tostring(AceLocale.appnames[self])..": Missing entry for '"..tostring(key).."'")
		return key
	end
}
AceLocale.MissingLinesElvUIChat = {}
-- end block
...
function AceLocale:NewLocale(application, locale, isDefault, silent)
	...
	if isDefault and (not app.defaultLocale or app.defaultLocale == 'defaultLocale') then -- ElvUIChat
...
--- Modified by ElvUIChat to add `locale` as second arg and the CopyTable section
