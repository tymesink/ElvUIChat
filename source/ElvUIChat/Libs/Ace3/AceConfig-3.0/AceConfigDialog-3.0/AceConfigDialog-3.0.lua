...
local reg = LibStub("AceConfigRegistry-3.0-ElvUIChat")

local MAJOR, MINOR = "AceConfigDialog-3.0-ElvUIChat", 87
...
AceConfigDialog.tooltip = AceConfigDialog.tooltip or CreateFrame("GameTooltip", "ElvUIChatAceConfigDialogTooltip", UIParent, "GameTooltipTemplate")
...
local function GetOptionsMemberValue(membername, option, options, path, appName, ...)
	...
	if ( not allIsLiteral[membername] ) and ( type(member) == "function" or ((not stringIsLiteral[membername]) and type(member) == "string") ) then
		...
		local a, b, c, d, e, f, g, h -- ElvUIChat adds e,f,g,h for default color
		...
		if not hidden then
			if v.type == "group" then
				...
				if v.type == "execute" then
					...
					control = CreateControl(v.dialogControl or v.control, iconControl and "Icon" or "Button-ElvUIChat")
					if iconControl then
						if not width then
				...
				elseif v.type == "input" then
					control = CreateControl(v.dialogControl or v.control, v.multiline and "MultiLineEditBox-ElvUIChat" or "EditBox")
				...
				elseif v.type == "range" then
					control = CreateControl(v.dialogControl or v.control, "Slider-ElvUIChat")
					control:SetLabel(name)
				...
				elseif v.type == "multiselect" then
					...
					if control then
						...
						for s = 1, #valuesort do
							...
							local text = values[value]
							if dragdrop then
								local button = gui:Create("Button-ElvUIChat")
				...
				elseif v.type == "color" then
					control = CreateControl(v.dialogControl or v.control, "ColorPicker-ElvUIChat")
					control:SetLabel(name)
